...
public class Calculator { // Calculator 클래스를 생성
    ...
    // 원주율 상수 선언, 상수는 변하지 않는 값을 의미며 final로 선언하여 값을 변경할 수 없게 만들어 신뢰할 수 있는 값을 만듭니다.
    private ArrayList<Integer> list; // 사칙연산 저장
    /*
    ...
    */
    private ArrayList<Double> circleArea; // 원주율 저장
    ...
    public Calculator() { // 생성자 생성.
        ...
        * 시도할 경우 에러가 발생합니다.
        */
        this.circleArea = new ArrayList<>(); // 생성자를 통해 원주율 배열을 초기화합니다.
    }

    // 사칙연산 메서드
    public int calculate(int num1, int num2, char operator) {
        /*
        // 반환 타입은 int로 설정하고 매개변수로 num1, num2, operator(연산자)를 받습니다.
    ...
    // 원의 넓이를 계산 후 반환해주는 메서드
    public double calculateCircleArea(double radius) {
        /* 반환 타입은 double로 설정하고 매개변수로 radius(반지름)를 받습니다.
        * static으로 선언하는 이유는 객체를 생성하지 않고도 사용할 수 있도록 하기 위함입니다.
        * private로 선언하는 이유는 다른 클래스에서 접근하지 못하도록 하기 위함입니다.
        * 다른 클래스에서 접근하지 못하도록 한다면 getter와 setter를 사용하여 간접 접근을 통해 필드에 접근할 수 있도록 구현할 수 있습니다.
        * 그래서하단에 getter와 setter를 추가해줍니다.
        */
        double area = PI * radius * radius; // 원의 넓이를 계산합니다. result를 적으면 위와 중복되니 변경해줍니다 area로 지은 이유는 원의 넓이를 의미???해서입니다
        circleArea.add(area); // 원의 넓이를 원주율 배열에 저장합니다.
        return area; // 원의 넓이를 반환합니다.
    }

    // 원주율 저장 필드 및 조회 메서드
    public ArrayList<Double> getCircleArea() {
        // 요구사항을 만족하기 간접 접근을 통해 필드에 접근하여 가져 올 수 있도록 구현하기 위해서 getter를 사용하여 원주율 배열을 가져옴
        // public ArrayList<Double> getCircleArea() { // 반환 타입 : ArrayList<Double> , getCircleArea = 원주율 배열을 가져옴 , 반환 값 : circleArea 배열
    ...
    public void setCircleArea(ArrayList<Double> circleArea) {
        ...
        this.circleArea = circleArea; // 원주율 배열을 변경합니다.
        // app에서 사용시 list.remove(0)을 사용하여 리스트 배열에 저장된 0,1,2,3과같은 수를 입력하여 인덱스 값에 따라 삭제할 수 있습니다.
    }
    public void inquiryCircleArea() { // 원주율 배열에 저장된 값을 조회하는 메서드를 추가
        // public void inquiryCircleArea() { // 반환 타입 : void , inquiryCircleArea = 원주율 배열에 저장된 값을 조회 , 매개변수 : 없음 ,
        // 반환 값 : 없음 , 반환값과 매개변수가 없는 이유는 원주율 배열에 저장된 값을 조회하기만 하기 때문입니다.
        if (!circleArea.isEmpty()) { // 조건 : 원주율 배열에 저장된 값이 없는 경우가 아닌 경우, !가 부정을 의미하기 떄문에 원주율 배열에 저장된 값이 있는 경우입니다.
            // isEmpty()는 원주율 배열이 비어있는지 확인하는 메서드 엠티 = 비어있다 = !부정.원주율.비어있다 원주율이 비어있는지에 대한 조건
            System.out.println("원주율 배열에 저장된 값 : " + circleArea); // 원주율 배열에 저장된 값을 출력합니다.
        }
        else {
            System.out.println("원주율 배열에 저장된 값이 없습니다!!!"); // 원주율 배열에 저장된 값이 없는 경우 if 에서 !가 없는경우와 같습니다.
        }
    }

    public void removeFirstCircleArea() { // 원주율 배열의 맨 앞의 값을 삭제하는 메서드를 추가
        // public void removeFirstCircleArea() { // 반환 타입 : void , removeFirstCircleArea = 원주율 배열의 맨 앞의 값을 삭제 , 매개변수 : 없음 ,
        // 반환 값 : 없음 , 반환값과 매개변수가 없는 이유는 원주율 배열의 맨 앞의 값을 삭제하기만 하기 때문입니다.
        if (!circleArea.isEmpty()) { // 조건 : 원주율 배열에 저장된 값이 없는 경우가 아닌 경우, !가 부정을 의미하기 떄문에 원주율 배열에 저장된 값이 있는 경우입니다.
            // isEmpty()는 원주율 배열이 비어있는지 확인하는 메서드 엠티 = 비어있다 = !부정.원주율.비어있다 원주율이 비어있는지에 대한 조건
            System.out.println("원주율 배열의 맨 앞의 값이 삭제되었습니다. 삭제된 값 : " + circleArea.get(0)); // 원주율 배열의 맨 앞의 값이 삭제된 경우
            circleArea.remove(0); // 원주율 배열에 저장된 맨 앞의 값을 삭제합니다.
            // 만약 원주율 맨 앞의값이 아닌 다른값을 삭제시키고 싶다면 circleArea.remove(0)을 circleArea.remove(1)로 변경
            // 즉, 안에 숫자는 인덱스를 의미합니다.
            System.out.println("현재 원주율 배열에 저장된 값 : " + circleArea); // 원주율 배열에 저장된 값을 출력합니다.
        }
        else {
            System.out.println("원주율 배열에 저장된 값이 없습니다!!!"); // 원주율 배열에 저장된 값이 없는 경우
        }
    }
